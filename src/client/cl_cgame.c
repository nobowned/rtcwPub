...
void CL_SetCGameTime( void ) {
	...
	if (clc.demoplaying && demo_paused->integer) {
		return;
	}
	...
	if ( clc.demoplaying && cl_freezeDemo->integer ) {
		...
		if (clc.demo_was_paused)
		{
			int pause_time_delta = cls.realtime - clc.demo_pause_time;

			cl.snap.serverTime += pause_time_delta;
			clc.demo_pause_delta += pause_time_delta;
			clc.demo_was_paused = qfalse;
		}
