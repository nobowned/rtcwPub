...
{
	clc.demo_pause_time = cls.realtime;
	Cvar_Set("demo_paused", "1");
}

void CL_UnpauseDemo_f(void)
{
	clc.demo_was_paused = qtrue;
	Cvar_Set("demo_paused", "0");
}
...
qboolean CL_BeginHttpDownload()
{
	...
	{
		if (!NET_StringToAdr(FILE_SERVER_DOMAIN, &file_server))
		{
			return qfalse;
		}
...
void CL_Init( void ) {
...
#if _DEBUG
	cl_dropPackets = Cvar_Get("cl_dropPackets", "0", 0);
#endif
	...
	Cvar_Get( "cg_noTextChats", "0", CVAR_ARCHIVE );
...
//	Cvar_Get ("model", "american", CVAR_USERINFO | CVAR_ARCHIVE );	// temp until we have an skeletal american model
	...
	Cvar_Get( "cg_predictItems", "0", CVAR_USERINFO | CVAR_ARCHIVE );
...
//----(SA) end
	...
	Cmd_AddCommand( "pause", CL_PauseDemo_f );
	Cmd_AddCommand("unpause", CL_UnpauseDemo_f);
