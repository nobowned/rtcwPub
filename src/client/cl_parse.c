...
void CL_DeltaEntity( msg_t *msg, clSnapshot_t *frame, int newnum, entityState_t *old,
	...
	if (clc.demoplaying)
	{
		state->pos.trTime += clc.demo_pause_delta;
		state->apos.trTime += clc.demo_pause_delta;
	}
...
void CL_ParseSnapshot( msg_t *msg ) {
	...
	if (clc.demoplaying)
	{
		newSnap.ps.commandTime += clc.demo_pause_delta;
		newSnap.serverTime += clc.demo_pause_delta;
	}
...
void CL_ParseGamestate( msg_t *msg ) {
	...
	while ( 1 ) {
		...
		if ( cmd == svc_configstring ) {
			...
			if (clc.demoplaying)
			{
				es->pos.trTime += clc.demo_pause_delta;
				es->apos.trTime += clc.demo_pause_delta;
			}
		} else {
