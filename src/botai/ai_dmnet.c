...
qboolean BotGetRouteGoal(bot_state_t *bs, bot_goal_t *goal) {
	bot_goal_t previous_goal;
	int seconds_doing_route;

	seconds_doing_route = 0;
	...
	if (goal) {
		memcpy(&previous_goal, goal, sizeof(bot_goal_t));
	}
	else {
		memset(&previous_goal, 0, sizeof(bot_goal_t));
	}
	//if the current goal expired
	if (bs->ltg_time < trap_AAS_Time()) {
		//pop the current goal from the stack
		if (previous_goal.flags & GFL_ROUTE_CAMP && bs->camp_time) {
			seconds_doing_route = trap_AAS_Time() - bs->camp_time;
		}
		trap_BotPopGoal(bs->gs);
		if (trap_BotChooseRouteGoal(bs->gs, bs->team, bs->origin, &previous_goal, seconds_doing_route) &&
			trap_BotGetTopGoal(bs->gs, goal)) {
			if (goal->flags & GFL_ROUTE_CAMP) {
				bs->ltgtype = LTG_CAMP;
				bs->teamgoal_time = trap_AAS_Time() + 5;
				memcpy(&bs->teamgoal, goal, sizeof(bot_goal_t));
				if (!(previous_goal.flags & GFL_ROUTE_CAMP)) {
				...
				bs->camp_time = 0;
				bs->ltg_time = trap_AAS_Time() + 30;
			}
			if (trap_AAS_Time() - bs->killedenemy_time < 2) {
				goal->flags &= ~GFL_WALK;
			}
			return qtrue;
		}
...
int AINode_Stand(bot_state_t *bs) {
	...
	// TODO(nobo): ATM stand ainode is used to wait for script_movers to finish.
	// Create new AINode for waiting, or remove it's ability to Talk if simply waiting
...
void AIEnter_Respawn(bot_state_t *bs) {
	...
	else if (EntityIsGibbable(&entinfo)) {
		bs->respawn_time = trap_AAS_Time() + 20;
	}
	else {
...
int AINode_Respawn(bot_state_t *bs) {
	...
	else if (bs->respawn_time < trap_AAS_Time() || !EntityIsGibbable(&entinfo)) {
	...
	else if (bs->cur_ps.viewlocked_entNum >= 0 && trap_AAS_Time() - bs->lastmedic_time > 11) {
		trap_EA_VoiceTeam(bs->client, "Medic");
		bs->lastmedic_time = trap_AAS_Time();
	}
...
int AINode_Seek_ActivateEntity(bot_state_t *bs) {
	...
	//if the bot touches the current goal
		...
		if (goal->flags & GFL_ACTIVATE) {
			if (bs->cur_ps.serverCursorHint != HINT_BUTTON) {
				VectorSubtract(goal->origin, bs->origin, dir);
			...
			bs->stand_time = trap_AAS_Time() + 8;
			AIEnter_Stand(bs);
			return qtrue;
		}
		else if (goal->flags & GFL_REVIVE) {
			BotEntityInfo(goal->entitynum, &entinfo);

			if (!EntityIsDead(&entinfo) || !EntityIsGibbable(&entinfo) || !BotSameTeam(bs, entinfo.number) || !entinfo.valid) {
				AIEnter_Seek_NBG(bs);
				return qfalse;
			}

			trap_EA_Crouch(bs->client);
			if (bs->cur_ps.serverCursorHint != HINT_REVIVE) {
				VectorSubtract(goal->origin, bs->origin, dir);
				vectoangles(dir, angles);
				VectorCopy(angles, bs->ideal_viewangles);
				return qtrue;
...
int AINode_Battle_Fight(bot_state_t *bs) {
	...
	if (!EntityIsGibbable(&entinfo) && BotWantsToRetreat(bs)) {
		AIEnter_Battle_Retreat(bs);
		return qtrue;
	}
	return qtrue;
...
int AINode_Battle_Retreat(bot_state_t *bs) {
	...
	if (EntityIsGibbable(&entinfo) || BotWantsToChase(bs)) {
	...
	if (moveresult.flags & (MOVERESULT_MOVEMENTVIEW
		...
		if (attack_skill > 0.3 && bs->enemyvisible_time >= trap_AAS_Time() - 1) {
			BotAimAtEnemy(bs);
		}
		else {
			if (trap_BotMovementViewTarget(bs->ms, &goal, bs->tfl, 300, target)) {
