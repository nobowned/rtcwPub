...
/*
==================
BotAI_GetEntityShared
==================
*/
int BotAI_GetEntityShared( int entityNum, entityShared_t *shared ) {
	gentity_t	*ent;
	...
	memcpy(shared, &ent->r, sizeof(entityShared_t));
	return qtrue;
}
...
void QDECL BotAI_BotInitialChat( bot_state_t *bs, char *type, ... ) {
	...
	// Nobo - Removing names for now. Breaks bot leader commands to teammates.
...
void BotInputToUserCommand(bot_input_t *bi, usercmd_t *ucmd, int delta_angles[3], int time) {
	...
	if (bi->actionflags & ACTION_ACTIVATE)
	...
	if (bi->actionflags & ACTION_RELOAD)
		ucmd->wbuttons |= g_dropReload.integer ? WBUTTON_DROP : WBUTTON_RELOAD;
	if (bi->actionflags & ACTION_SPRINT) {
	...
	if (bi->actionflags & ACTION_MOVERIGHT)
		ucmd->rightmove += 127;
	//jump/moveup
	if (bi->actionflags & ACTION_JUMP || bi->actionflags & ACTION_RESPAWN)
		ucmd->upmove += 127;
	//crouch/movedown
...
void BotUpdateInput(bot_state_t *bs, int time) {
	...
	//pack hack
	if (bs->weaponnum == WP_MEDIC_SYRINGE && bs->revive_time <= trap_AAS_Time() - 2) {
		BotChooseNewWeapon(bs);
	}
	//needle hack
	if (bs->weaponnum == WP_MEDKIT && bs->inventory[INVENTORY_HEALTH] >= 100) {
		BotChooseNewWeapon(bs);
	}
	//
...
int BotAI(int client, float thinktime) {
	...
	while( trap_BotGetServerCommand(client, buf, sizeof(buf)) ) {
		...
		*args++ = '\0';
		...
		if (!Q_stricmp(buf, "cp "))
			...
			trap_BotQueueConsoleMessage(bs->cs, CMS_VOICE, args);
	...
	//wolf has teams.. always..
	bs->team = level.clients[client].sess.sessionTeam;
	//the real AI
