...
void BotTransformVoiceToChat(char *message)
{
	char msg[150];
	char *ptr;
	char *endvoice;

	ptr = strchr(message, ' ') + 1;

	ClientName(atoi(ptr), msg, sizeof(msg));
	strcat(msg, ": ");
...
void BotChooseNewWeapon(bot_state_t *bs) {
...
int WeaponCheck(bot_state_t *bs, int weapon)
{
	return COM_BitCheck(bs->cur_ps.weapons, weapon) ? 1 : -1;
}

int AmmoCheck(bot_state_t *bs, int weapon)
{
	int totalammo = bs->cur_ps.ammo[BG_FindAmmoForWeapon(weapon)] + bs->cur_ps.ammoclip[BG_FindClipForWeapon(weapon)];
	return totalammo ? totalammo : -1;
}

int ChargeCheck(bot_state_t *bs)
{
	int chargeTime;

	if (bs->cur_ps.stats[STAT_PLAYER_CLASS] == PC_ENGINEER) {
		chargeTime = g_engineerChargeTime.value;
	}
	else if (bs->cur_ps.stats[STAT_PLAYER_CLASS] == PC_MEDIC) {
		chargeTime = g_medicChargeTime.value;
	}
	else if (bs->cur_ps.stats[STAT_PLAYER_CLASS] == PC_LT) {
	...
	return (level.time - bs->cur_ps.classWeaponTime >= chargeTime * 0.25f) ? 1 : -1;
}
...
void BotUpdateInventory(bot_state_t *bs) {
	//weapons
	bs->inventory[INVENTORY_LUGER] = WeaponCheck(bs, WP_LUGER);
	bs->inventory[INVENTORY_COLT] = WeaponCheck(bs, WP_COLT);
	bs->inventory[INVENTORY_MAUSER] = WeaponCheck(bs, WP_MAUSER);
	bs->inventory[INVENTORY_MP40] = WeaponCheck(bs, WP_MP40);
	bs->inventory[INVENTORY_THOMPSON] = WeaponCheck(bs, WP_THOMPSON);
	bs->inventory[INVENTORY_PANZERFAUST] = WeaponCheck(bs, WP_PANZERFAUST);
	bs->inventory[INVENTORY_GRENADE_LAUNCHER] = WeaponCheck(bs, WP_GRENADE_LAUNCHER);
	bs->inventory[INVENTORY_VENOM] = WeaponCheck(bs, WP_VENOM);
	bs->inventory[INVENTORY_FLAMETHROWER] = WeaponCheck(bs, WP_FLAMETHROWER);
	bs->inventory[INVENTORY_KNIFE] = WeaponCheck(bs, WP_KNIFE);
	// TODO(nobo): INVENTORY_CHARGE
	bs->inventory[INVENTORY_MEDKIT] = ChargeCheck(bs);
	bs->inventory[INVENTORY_SYRINGE] = WeaponCheck(bs, WP_MEDIC_SYRINGE);

	// ammo
	bs->inventory[INVENTORY_9MM_PISTOL] = AmmoCheck(bs, WP_LUGER);
	bs->inventory[INVENTORY_45MM_PISTOL] = AmmoCheck(bs, WP_COLT);
	bs->inventory[INVENTORY_45MM] = AmmoCheck(bs, WP_THOMPSON);
	bs->inventory[INVENTORY_9MM] = AmmoCheck(bs, WP_MP40);
	bs->inventory[INVENTORY_792MM] = AmmoCheck(bs, WP_MAUSER);
	bs->inventory[INVENTORY_GRENADES] = AmmoCheck(bs, WP_GRENADE_LAUNCHER);
	bs->inventory[INVENTORY_127MM] = AmmoCheck(bs, WP_VENOM);
	bs->inventory[INVENTORY_FUEL] = AmmoCheck(bs, WP_FLAMETHROWER);
	bs->inventory[INVENTORY_SYRINGES] = AmmoCheck(bs, WP_MEDIC_SYRINGE);
	bs->inventory[INVENTORY_ROCKETS] = AmmoCheck(bs, WP_PANZERFAUST);
...
==================
*/
qboolean BotUsePacks(bot_state_t *bs) {
	if (bs->inventory[INVENTORY_HEALTH] < 100 &&
		bs->inventory[INVENTORY_MEDKIT] &&
		bs->enemyvisible_time < trap_AAS_Time() - 4)
	{
		if (bs->weaponnum != WP_MEDKIT) {
...
==============
*/
void BotCheckReload(bot_state_t *bs, float mult) {
	if ((bs->cur_ps.ammoclip[BG_FindClipForWeapon(bs->cur_ps.weapon)] < (int)(mult*ammoTable[bs->cur_ps.weapon].maxclip))
...
qboolean EntityIsGibbable(aas_entityinfo_t *entinfo) {
	gentity_t *ent;

	if (entinfo->number >= 0 && entinfo->number < MAX_CLIENTS) {
		ent = &g_entities[entinfo->number];

		if (ent->takedamage && ent->r.contents == CONTENTS_CORPSE && !(ent->r.contents & CONTENTS_NODROP) && !(ent->client->ps.pm_flags & PMF_LIMBO))
			return qtrue;
	}
	return qfalse;
}

qboolean GetHeadPosition(aas_entityinfo_t *entinfo, vec3_t head_position) {
	gentity_t *ent;
	orientation_t ori;

	if (entinfo->number >= 0 && entinfo->number < MAX_CLIENTS) {
		ent = &g_entities[entinfo->number];
		if (trap_GetTag(ent, &ent->client->animationInfo, "tag_head", &ori)) {
			VectorCopy(ori.origin, head_position);
			return qtrue;
		}
	}
...
qboolean EntityIsInvisible(aas_entityinfo_t *entinfo) {
	...
	if (entinfo->number >= 0 && entinfo->number < MAX_CLIENTS) {
		...
		if (entinfo->powerups & (1 << PW_INVIS) || ent->s.eType == ET_INVISIBLE) {
			return qtrue;
		}
	}
	return qfalse;
...
==================
*/
qboolean EntityIsMoving(aas_entityinfo_t *entinfo) {
	playerState_t ps;

	if (entinfo->number >= 0 && entinfo->number < MAX_CLIENTS) {
		//retrieve the current client state
		BotAI_GetClientState(entinfo->number, &ps);
		if (VectorLength(ps.velocity)) return qtrue;
	}
	return qfalse;
}
...
==================
*/
qboolean EntityIsWalking(aas_entityinfo_t *entinfo) {
	gclient_t *cl;

	if (entinfo->number >= 0 && entinfo->number < MAX_CLIENTS) {
		cl = level.clients + entinfo->number;
		if (cl->pers.cmd.buttons & BUTTON_WALKING) return qtrue;
	}
	return qfalse;
}
...
==================
*/
int TeamPlayIsOn(bot_state_t *bs) {
	return ( gametype >= GT_TEAM && bs->numteammates > 1 );
}
...
float BotAggression(bot_state_t *bs) {
	...
	// Nobo - commented becasuse lower ground is actually better in wolf (when crouching)
	//if (bs->inventory[ENEMY_HEIGHT] > 200) return 0;
	//if the bot is very low on health
	if (bs->inventory[INVENTORY_HEALTH] < 30) return 0;
	//if the bot can use the flamer
	if (bs->inventory[INVENTORY_FLAMETHROWER] > 0 &&
		bs->inventory[INVENTORY_FUEL] > 50) return 90;
	//if the bot can use the panzer
	if (bs->inventory[INVENTORY_PANZERFAUST] > 0 &&
		bs->inventory[INVENTORY_ROCKETS] != 0) return 90;
	//if the bot can use the venom
	if (bs->inventory[INVENTORY_VENOM] > 0 &&
		bs->inventory[INVENTORY_127MM] > 50) return 90;
	//if the bot can use the mp40/thompson
	if (bs->inventory[INVENTORY_9MM] > 0 || bs->inventory[INVENTORY_45MM] > 0)
		return 51;
...
int BotCanAndWantsToRocketJump(bot_state_t *bs) {
	...
	if (bs->inventory[INVENTORY_PANZERFAUST] <= 0) return qfalse;
	//if low on rockets
...
int BotWantsToCamp(bot_state_t *bs) {
	...
	if ((bs->inventory[INVENTORY_PANZERFAUST] <= 0 || bs->inventory[INVENTORY_ROCKETS < 10])
...
bot_moveresult_t BotAttackMove(bot_state_t *bs, int tfl) {
	...
	gibbing = EntityIsDead(&entinfo) && EntityIsGibbable(&entinfo);
	...
	if (!gibbing) {
		...
		if (bs->attackcrouch_time > trap_AAS_Time() || bs->enemyfar) movetype = MOVE_CROUCH;
...
float BotEntityVisible(int viewer, vec3_t eye, vec3_t viewangles, float fov, int ent) {
	...
	for (i = 0; i < 3; i++) {
		...
		//
		contents_mask = CONTENTS_SOLID|CONTENTS_PLAYERCLIP|CONTENTS_CORPSE;
		passent = viewer;
		hitent = ent;
...
int BotFindEnemy(bot_state_t *bs, int curenemy) {
	...
	for (i = 0; i < MAX_CLIENTS; i++) {
		...
		// TODO(nobo): Perhaps tie gibbing dead enemies to a spearate
		// AINode altogether..? Currently the bot will treat gibbing
		// as if they're in battle with a corpse.. which looks.. off
		//if the enemy isn't dead or is gibbable
		if (EntityIsDead(&entinfo) && !EntityIsGibbable(&entinfo)) continue;
		//if the enemy isn't the bot self
		...
		// TODO(nobo): Fine tune this, there has to be a legitimate distance that
		// is too far for human players, and then can be emulated here.
		//if the bot is extremely far away (increased from 900 to 4000 for wolfenstein)
		if (dist > 4000 + alertness * 4000) continue;
		//if on the same team
		if (BotSameTeam(bs, i)) continue;
		//if the bot's health decreased or the enemy is shooting
		if (curenemy < 0 && (healthdecrease || EntityIsShooting(&entinfo))) fov = 360;
		//if the enemy is closeby and moving but not walking (bot can "hear" them)
		else if (curenemy < 0 && dist < 500 && EntityIsMoving(&entinfo) && !EntityIsWalking(&entinfo)) fov = 360;
...
/*
==================
BotRevivableTeammate
==================
*/
qboolean BotRevivableTeammate(bot_state_t *bs)
{
	int i, areanum;
	float fov, dist, vis;
	aas_entityinfo_t entinfo;
	...
	if (bs->cur_ps.stats[STAT_PLAYER_CLASS] != PC_MEDIC || bs->inventory[INVENTORY_SYRINGES] <= 0)
		return qfalse;
	...
	for (i = 0; i < MAX_CLIENTS; i++) {
		...
		//
		if (!entinfo.valid) continue;
		//if the enemy isn't dead or isn't gibbable (revivable)
		if (!EntityIsDead(&entinfo) || !EntityIsGibbable(&entinfo)) continue;
		//if the player is the bot self
		if (entinfo.number == bs->entitynum) continue;
		//calculate the distance towards the fallen player
		VectorSubtract(entinfo.origin, bs->origin, dir);
		dist = VectorLength(dir);
		//if the bot is extremely far away
		...
		areanum = trap_BotReachabilityArea(entinfo.origin, bs->client);
		if (!areanum) continue;

		// Set found player as activate goal
		VectorCopy(entinfo.origin, bs->activategoal.origin);
		// Corpse is a bit below the player's origin
		bs->activategoal.origin[2] -= 22;
		VectorCopy(entinfo.mins, bs->activategoal.mins);
		VectorCopy(entinfo.maxs, bs->activategoal.maxs);
		bs->activategoal.areanum = areanum;
		bs->activategoal.entitynum = entinfo.number;
		bs->activategoal.number = 0;
		bs->activategoal.flags = GFL_REVIVE;
...
void BotAimAtEnemy(bot_state_t *bs) {
	int i, enemyvisible;
	float dist, f, aim_skill, aim_accuracy, speed, reactiontime, alertness;
	...
	BotEntityInfo(bs->enemy, &entinfo);
	//entity is floating in limbo
	if (EntityIsDead(&entinfo) && !EntityIsGibbable(&entinfo)) return;
	...
	//better aim when crouching
	if (bs->cur_ps.eFlags & EF_CROUCHING) {
		aim_accuracy += aim_accuracy * aim_accuracy;
		aim_accuracy = min(aim_accuracy, 1);
	}
	...
	if (enemyvisible) {
		//
#if 0
		if (!GetHeadPosition(&entinfo, bestorigin)) {
		...
		VectorSubtract(entinfo.origin, bs->origin, dir);
		dist = VectorLength(dir);
		if (dist > 900 + alertness * 4000) bs->enemyfar = qtrue;
		else bs->enemyfar = qfalse;
...
void BotCheckAttack(bot_state_t *bs) {
	...
	//
	BotEntityInfo(bs->enemy, &entinfo);
	if (EntityIsDead(&entinfo) && !EntityIsGibbable(&entinfo)) {
		return;
	}
	// Knife shenanigans
	...
	//check if medpack out..
	if (bs->weaponnum == WP_MEDKIT) {
		BotChooseNewWeapon(bs);
		return;
	}
	//reload now..
	BotCheckReload(bs, 0.10f);
	//get the weapon info
	trap_BotGetWeaponInfo(bs->ws, bs->weaponnum, &wi);
	//get the start point shooting from
	VectorCopy(bs->origin, start);
...
int BotEntityToActivate(int entitynum) {
	...
	char targetname[128], classname[128];
	float health;
	...
	if (!trap_AAS_ValueForBSPEpairKey(ent, "targetname", targetname, sizeof(targetname))) {
#ifdef OBSTACLEDEBUG
		BotAI_Print(PRT_ERROR, "BotEntityToActivate: entity with model \"%s\" has no targetname\n", model);
#endif //OBSTACLEDEBUG
		return 0;
	}
	if (!strcmp(classname, "script_mover")) {
		if (strstr(targetname, "gate")) {
			return ent;
		}
	}
...
void BotAIBlocked(bot_state_t *bs, bot_moveresult_t *moveresult, int activate) {
	int i, ent, movetype;
	char classname[128], targetname[128]; //, model[128];
	vec3_t hordir, /*size,*/ start, end, mins, maxs, sideward, angles, up, forward;
	aas_entityinfo_t entinfo;
	vec3_t range = { 320, 320, 320 };
	int entList[20];
	gentity_t *entity;
	gentity_t *bestlever = NULL;
	int dist, bestdist = 999999;
	int areanum;
#ifdef OBSTACLEDEBUG
	char buf[128];
#endif
...
//		BotAI_Print(PRT_MESSAGE, "%s: I've got no brain cells for activating entities\n", netname);
		...
		if (!strcmp(classname, "script_mover")) {
			trap_AAS_ValueForBSPEpairKey(ent, "targetname", targetname, sizeof(targetname));

			// Blocked by a gate? Let's try and open it
			if (!strstr(targetname, "gate"))
				return;

			// It's already opened, we must have hit the chunk that sticks out
			if (g_entities[ent].moverState == MOVER_POS2) {
				// TODO(nobo): There should be an invisible func_static that tells the gate where it's destination should be.
				// Find it and calculate the direction between the two. Use dir to find out which way we should strafe and how far
			}
			// It's opening, wait it out
			else if (g_entities[ent].moverState == MOVER_1TO2) {
				bs->stand_time = trap_AAS_Time() + (g_entities[ent].s.pos.trDuration / 100);
				AIEnter_Stand(bs);
				return;
			}

			// Find the closest lever within a 10 meter radius
			VectorSubtract(bs->cur_ps.origin, range, mins);
			VectorAdd(bs->cur_ps.origin, range, maxs);
			int hit = trap_EntitiesInBox(mins, maxs, entList, 20);;

			for (i = 0; i < hit; i++) {
				entity = g_entities + entList[i];

				if (!entity->targetname || !entity->targetname[0])
					continue;

				if (entity->s.eType == ET_MOVER && strstr(entity->targetname, "lever")) {
					dist = Distance(bs->cur_ps.origin, entity->r.currentOrigin);
					if (dist < bestdist) {
						bestdist = dist;
						bestlever = entity;
					}
			...
			// Lever origin is a bit inside the wall
			AngleVectors(bs->viewangles, forward, NULL, NULL);
			VectorNegate(forward, forward);
			VectorMA(bestlever->r.currentOrigin, 15, forward, end);

			areanum = trap_BotReachabilityArea(end, bs->client);

			if (!areanum)
				return;
			
			// Set found lever as activate goal
			VectorCopy(end, bs->activategoal.origin);
			bs->activategoal.origin[2] -= 20;
			bs->activategoal.areanum = areanum;
			VectorCopy(bestlever->r.mins, bs->activategoal.mins);
			VectorCopy(bestlever->r.maxs, bs->activategoal.maxs);
			bs->activategoal.entitynum = bestlever - g_entities;
			bs->activategoal.number = 0;
			bs->activategoal.flags = GFL_ACTIVATE;
			bs->activate_time = trap_AAS_Time() + 10;
...
void BotCheckConsoleMessages(bot_state_t *bs) {
	...
	while((handle = trap_BotNextConsoleMessage(bs->cs, &m)) != 0) {
		...
		if (m.type == CMS_CHAT) {
			// Remove localization argument
			char *ptr = strrchr(m.message, '"');
			if (ptr && *ptr) *ptr = 0;

			// Remove localization formatting - [lof] and [lon]
			Q_RemoveLocalization(m.message);
		}
		else if (m.type == CMS_VOICE) {
			BotTransformVoiceToChat(m.message);
