...
#pragma warning(disable : 4706)		// Variable assigned inside of conditional statement
...
#define	MAX_NAME_LENGTH		32		// max length names (hostname, gamename)
#define MAX_NETNAME			36		// max length of client name
...
#define PROTOCOL_VERSION_1_4 60

#ifndef PRE_RELEASE_DEMO
#define PROTOCOL_VERSION PROTOCOL_VERSION_1_0
...
#define Q_IsColorString2(p)	( (p) && *(p) == Q_COLOR_ESCAPE && *((p)+1) > 0 && isalnum(*((p)+1)) ) // ^[0-9a-zA-Z]
...
//----(SA)	end
	...
	qboolean	noReload;		// A no-reload hac
...
// Logs
#define CRITICALLOG "./logs/critical.log"
...
#define USAGELOG	"./logs/usageTracking.log"
#define LOGLINE		"\n----------------------\n"
