...
=================
*/
void BG_ParseConditionBits( char **text_pp, animStringItem_t *stringTable, int condIndex, int *result, size_t result_size )
{
...
qboolean BG_ParseConditions( char **text_pp, animScriptItem_t *scriptItem )
{
	...
	while (1) {
		...
		case ANIM_CONDTYPE_BITFLAGS:
			BG_ParseConditionBits( text_pp, animConditionsTable[conditionIndex].values, conditionIndex, conditionValue, sizeof(conditionValue) );
			break;
	...
	while (1) {
		...
		case PARSEMODE_DEFINES:
			...
			if ( !Q_stricmp( token, "set" ) ) {
				...
				// parse the bits
				BG_ParseConditionBits( &text_p, animConditionsTable[defineType].values, defineType, defineBits[defineType][numDefines[defineType]], sizeof(defineBits[defineType][numDefines[defineType]]));
...
int BG_GetAnimationDuration(playerState_t *ps, scriptAnimEventTypes_t eventType)
{
	animModelInfo_t		*modelInfo;
	animScript_t		*script;
	animScriptItem_t	*scriptItem;
	...
	modelInfo = BG_ModelInfoForClient(ps->clientNum);
	script = &modelInfo->scriptEvents[eventType];
	if (!script->numItems)
	{
		return -1;
	}
	...
	return scriptCommand->animDuration[0] + 50;
}
