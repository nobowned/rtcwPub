...
==============
*/
extern void GibEntity ( gentity_t * self , int killer, qboolean explode ) ;
...
void grabber_die(gentity_t *ent, gentity_t *inflictor, gentity_t *attacker, int damage, int mod) {
	...
	GibEntity( ent, 0, qtrue );	// use temporarily to show 'death' of entity
...
void flakPuff (vec3_t origin, vec3_t forward, qboolean sky)
{
	gentity_t *tent;
	vec3_t		point;
...
void mg42_muzzleflash(gentity_t *ent) {
	...
	if (flash) {
		...
		AngleVectors(flash->s.apos.trBase, forward_local, NULL, NULL);
		VectorMA(point, 40, forward_local, point);

		SnapVector(point);
		G_SetOrigin(flash, point);
		SnapVector(flash->s.apos.trBase);

		flash->s.modelindex = G_ModelIndex("models/weapons2/machinegun/mg42_flash.md3");
...
void Fire_Lead(gentity_t *ent, gentity_t *activator, float spread, int damage,
	...
	qboolean owner_is_1_4_client = activator && activator->client && trap_GetClientProtocol(activator - g_entities) == PROTOCOL_VERSION_1_4;
...
// jpw
	...
	if (traceEnt->takedamage) {
		if (g_automg42.integer && ent->parent) {
			G_Damage(traceEnt, ent->parent, ent->parent, forward, tr.endpos, damage, 0, MOD_MACHINEGUN);
		} else {
...
{
	vec3_t	forward_local, right_local, up_local;
	vec3_t	point;

	AngleVectors (self->s.apos.trBase, forward_local, right_local, up_local);
	VectorMA (self->r.currentOrigin, -36, forward_local, point);
...
void mg42_touch(gentity_t *self, gentity_t *other, trace_t *trace) {
	...
	if (other->active) {
		...
		other->client->ps.viewlocked = trap_GetClientProtocol(other - g_entities) == PROTOCOL_VERSION_1_4 ? 3 : 1;
...
void mg42_fire(gentity_t *other) {
	...
	Fire_Lead(self, other, MG42_SPREAD_MP, MG42_DAMAGE_MP, muzzle, forward, right, up, qfalse);
}
...
void miscGunnerThink(gentity_t *ent)
{
	...
	if (fire) {
		...
		if (fabs( AngleNormalize180(gun->r.currentAngles[YAW] - gun->TargetAngles[YAW]) ) < 10) {
			...
			if (gun->damage)
				Fire_Lead (gun, gun, MG42_SPREAD/gun->accuracy, gun->damage, muzzle, forward, right, up, qfalse);
			else
				Fire_Lead (gun, gun, MG42_SPREAD/gun->accuracy, MG42_DAMAGE_AI, muzzle, forward, right, up, qfalse);
		}
	}
...
{
	gentity_t *left_ent, *right_ent, *airplane;
	...
	// left fire trail
	left_ent = G_Spawn();
	left_ent->classname = "left_firetrail";
	left_ent->r.contents = 0;
	left_ent->s.eType = ET_RAMJET;
	left_ent->s.modelindex = G_ModelIndex("models/ammo/rocket/rocket.md3");
	left_ent->tagParent = airplane;
	left_ent->tagName = "tag_engine1";	// tag to connect to
	left_ent->use = firetrail_use;
	left_ent->AIScript_AlertEntity = firetrail_die;
	left_ent->targetname = ent->targetname;
	G_ProcessTagConnect(left_ent);
	trap_LinkEntity(left_ent);

	// right fire trail
	right_ent = G_Spawn();
	right_ent->classname = "right_firetrail";
	right_ent->r.contents = 0;
	right_ent->s.eType = ET_RAMJET;
	right_ent->s.modelindex = G_ModelIndex("models/ammo/rocket/rocket.md3");
	right_ent->tagParent = airplane;
	right_ent->tagName = "tag_engine2";	// tag to connect to
	right_ent->use = firetrail_use;
	right_ent->AIScript_AlertEntity = firetrail_die;
	right_ent->targetname = ent->targetname;
	G_ProcessTagConnect(right_ent);
	trap_LinkEntity(right_ent);
...
{
	ent->s.eType = ET_ITEM;
	ent->s.modelindex = BG_FindItemForWeapon(WP_AMMO) - bg_itemlist;	// store item number in modelindex
