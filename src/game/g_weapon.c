...
//			if(ent->s.aiState == AISTATE_RELAXED) {
				damage = 140;	// enough to drop a 'normal' (100 health) human with one jab
...
//				ent->client->ps.classWeaponTime = level.time;
		...
		// clamp pitch
		if (g_clampPacketAngles.integer) {
			if (angles[PITCH] < -30)
				angles[PITCH] = -30;
...
void Weapon_MagicAmmo(gentity_t *ent) {
	...
	{
		if (level.time - ent->client->ps.classWeaponTime >= g_medicChargeTime.integer*0.75)
		{
			if (level.time - ent->client->ps.classWeaponTime > g_medicChargeTime.integer)
			{
				ent->client->ps.classWeaponTime = level.time - g_medicChargeTime.integer;
			...
			ent->client->ps.classWeaponTime += g_medicChargeTime.integer*0.75;
		}
		else
		{
			return;
...
void Weapon_Syringe(gentity_t *ent) {
	...
	// For testing other dead entities if we hit an enemy initially.
		...
		if (traceEnt->client != NULL) {
			if (traceEnt->client->ps.pm_type == PM_DEAD) {
				if (traceEnt->client->sess.sessionTeam != ent->client->sess.sessionTeam) {
					VectorMA(muzzleTrace, 256, forward, end);			// CH_ACTIVATE_DIST
					traceEnt = FindRevivableTeammate(ent, traceEnt->s.clientNum, forward, muzzleTrace, tr.endpos, end);
					if (traceEnt == NULL) {
				...
				if (g_unstuckRevive.integer) {
					if (tr.allsolid) {
						VectorCopy(traceEnt->client->ps.maxs, maxs);
						maxs[2] = traceEnt->client->ps.crouchMaxZ;
						trap_Trace(&tr, traceEnt->client->ps.origin, traceEnt->client->ps.mins, maxs, traceEnt->client->ps.origin, traceEnt->s.number, MASK_PLAYERSOLID);
						if (tr.allsolid && tr.entityNum >= MAX_CLIENTS) {
							traceEnt->client->revivedInsideOfTheWorld = qtrue;
							VectorCopy(ent->client->ps.origin, traceEnt->client->unstuckPos);
						}
					}
				}
				trap_LinkEntity( ent );
				...
				traceEnt->client->in_revived_state = qtrue;

				if (g_reviveReports.integer == 1) {
					AP(va("print \"%s ^7was revived by %s\n\"", traceEnt->client->pers.netname, ent->client->pers.netname));
				}
				else if (g_reviveReports.integer == 2) {
				...
				{
					CP(va("chat \"console: %s, ^7Your ^3TK ^7was forgiven!\n\"", ent->client->pers.netname));
					ent->client->pers.sb_TKforgiven++;
				}
			}

			else if ((traceEnt->client->ps.stats[STAT_HEALTH] > 0) &&
				(traceEnt->client->sess.sessionTeam != ent->client->sess.sessionTeam || g_gamestate.integer == GS_WARMUP_COUNTDOWN) &&
...
//	int			mod = MOD_KNIFE;
	...
	traceEnt = &g_entities[ tr.entityNum ];
	if (!Q_stricmp(traceEnt->classname, "misc_mg42") || !Q_stricmp(traceEnt->classname, "mg42_base")) {

		if (g_automg42.integer)
		{
			G_Automg42Repair(ent, traceEnt);
			return;
		}

		if (traceEnt->s.frame != 2)
		{
			return;
		}
		...
		if (traceEnt->health >= (MG42_MULTIPLAYER_HEALTH * MG42_FIXED_HEALTH_MULT)) {
			traceEnt->s.frame = 0;
			...
			CP("cp \"You have repaired the MG42!\n\"");
...
void Weapon_Artillery(gentity_t *ent) {
	...
	if (!g_ffa.integer && ent->client->ps.stats[STAT_PLAYER_CLASS] != PC_LT) {
...
// build arty falling sound effect in front of bomb drop
		...
		if (g_ffa.integer)
		{
			...
			ent->client->ps.ammoclip[BG_FindClipForWeapon(WP_ARTY)] = 0;
...
void DisableInvulnerability(gentity_t *ent)
{
...
void Bullet_Fire (gentity_t *ent, float spread, int damage ) {
	...
	if (g_disableInv.integer) {
		DisableInvulnerability(ent);
	}

	if (ent->client) {
		// antilag lerp if enough delay between client and server.
		if (g_antilag.integer && !(ent->client->sess.colorFlags & CF_ANTILAG_OFF) && !(ent->r.svFlags & SVF_BOT)) {
	...
	if (attacker && attacker->client) {
		if (target && target->client) {
			if (target->client->ps.stats[STAT_HEALTH] > 0 ||
				(OnSameTeam(attacker, target) && !g_ffa.integer)) {
				if ((target->client->ps.powerups[PW_INVULNERABLE] <= level.time &&
					!target->client->revive_animation_playing)) {
					return qtrue;
				}
			}
		}
...
//----(SA)	end
			...
			if (!g_headshotsOnly.integer || !traceEnt->client || (g_headshotsOnly.integer && traceEnt->headshot)) {
...
qboolean LogAccuracyHit( gentity_t *target, gentity_t *attacker ) {
	...
	if ( OnSameTeam( target, attacker ) && !g_ffa.integer ) {
		return qfalse;
	}
...
//			VectorMA( muzzlePoint, 16, right, muzzlePoint );
		...
		{
			int scale = g_automg42.integer && ent->client->ps.leanf ? Com_Clamp(-20, 20, ent->client->ps.leanf * 20) : 20;
			VectorMA(muzzlePoint, scale, right, muzzlePoint);
		} break;
		default:
