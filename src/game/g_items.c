...
void Add_Ammo (gentity_t *ent, int weapon, int count, qboolean fillClip)
{
	...
	if (count >= 999 && ammoTable[ammoweap].maxammo <= 999) { // 'really, give /all/'
		ent->client->ps.ammo[ammoweap] = count;
	}	// JPW NERVE
}
//----(SA)	end
...
int Pickup_Weapon (gentity_t *ent, gentity_t *other) {
	...
	int			ammoweap, clipweap;
	qboolean	has_primary = qfalse;
	int			clip_ammo;
...
// JPW NERVE -- magic ammo for any two-handed weapon
	if (ent->item->giTag == WP_AMMO) 
	{
		...
		// Don't give '' for ammo spawns
		// Don't allow medics to give themselves nades or throwing knives (g_medicAmmoPack)
		if (!g_ffa.integer &&
			!ent->wait && // ammo spawns have a wait (respawn time) set
			!(g_medicAmmoPack.integer &&
		...
		if (!g_ffa.integer)
		{
			...
			{
				if (other->client->ps.ammoclip[BG_FindAmmoForWeapon(WP_MEDIC_SYRINGE)] < g_syringes.integer)
		...
		other->client->ps.ammo[ammoweap] += ammoTable[weapon].maxclip;
		if ((other->client->ps.ammo[ammoweap] + other->client->ps.ammoclip[clipweap]) > ammoTable[ammoweap].maxammo)
			other->client->ps.ammo[ammoweap] = ammoTable[ammoweap].maxammo - other->client->ps.ammoclip[clipweap];
		...
		for (i = 0; i < MAX_WEAPS_IN_BANK_MP; ++i) {
			...
			if (COM_BitCheck(other->client->ps.weapons, weapon)) {
				...
				if (weapon == WP_FLAMETHROWER) // FT doesn't use magazines so refill tank
					...
					if (weapon == WP_MP40 || weapon == WP_THOMPSON || weapon == WP_STEN)
						clip_ammo = ent->count ? ent->count : ammoTable[weapon].maxclip;
					else
						clip_ammo = ammoTable[weapon].maxclip;

					other->client->ps.ammo[ammoweap] += clip_ammo;
					// Nobo - set to maxammo.. this way all of the extra clip cvars work with ammo packs as well..
					if ((other->client->ps.ammo[ammoweap] + other->client->ps.ammoclip[clipweap]) > ammoTable[ammoweap].maxammo)
						other->client->ps.ammo[ammoweap] = ammoTable[ammoweap].maxammo - other->client->ps.ammoclip[clipweap];
				}
			}
		...
		if (!has_primary && g_ammoPackGivesWeapon.integer)
		{
			int weapon = other->client->sess.selectedWeapon != WP_NONE ?
				other->client->sess.selectedWeapon : other->client->pers.playerWeapon;
...
// JPW NERVE did this so ammocounts work right on dropped weapons
	...
	if (ent->item->giTag == WP_BINOCULARS) {
		...
		COM_BitSet(other->client->ps.weapons, WP_ARTY);
		other->client->ps.ammo[BG_FindAmmoForWeapon(WP_ARTY)] = 0;
		other->client->ps.ammoclip[BG_FindClipForWeapon(WP_ARTY)] = 1;
	}
...
void Touch_Item (gentity_t *ent, gentity_t *other, trace_t *trace) {
	...
	if (g_dropAmmoPack.integer || g_ammoPackGivesWeapon.integer) {
		int weapon = PrimarySlotInUse(other->client->ps.weapons);

		if (weapon && ent->item->giTag == WP_AMMO && ent->count) {
			modelindex = BG_FindItemForWeapon(weapon) - bg_itemlist;
		}
	}

	// send the pickup event
	if (g_ffa.integer) {
		// don't auto-switch to picked up nades. it's annoying af
		if (ent->item->giTag == WP_GRENADE_LAUNCHER) {
			APS("sound/misc/w_pkup.wav");
		}
		else {
			G_AddEvent(other, makenoise, modelindex);
		}
	}
	else {
		if (other->client->pers.predictItemPickup) {
			G_AddPredictableEvent(other, makenoise, modelindex);
		}
		else {
			G_AddEvent(other, makenoise, modelindex);
		}
...
qboolean WeaponIsPrimary(int weapon)
{
	...
	for (i = 0; i < MAX_WEAPS_IN_BANK_MP; i++) {
		if (weapon == weapBanksMultiPlayer[3][i]) {
			return qtrue;
		}
	}
...
{
	gentity_t *parent = self->parent;
	
	if (other != self->parent &&
		level.time < self->timestamp &&
		parent->client->pers.connected == CON_CONNECTED &&
		(parent->client->sess.sessionTeam == TEAM_RED || parent->client->sess.sessionTeam == TEAM_BLUE) &&
		!(parent->client->ps.pm_flags & PMF_LIMBO) &&
		!PrimarySlotInUse(parent->client->ps.weapons))
	{
		return;
	}

	Touch_Item_Auto(self, other, tr);
}
