...
static gentity_t max_entity_failure;
...
gentity_t *G_Spawn( void ) {
	...
	if ( i == ENTITYNUM_MAX_NORMAL ) {
		if (level.time - last_print_time >= 2000) {
			trap_LogToFile(CRITICALLOG, va("Time: %s\nServer ran out of entities.%s", getDateTime(), LOGLINE));
			AP("print \"Server ran out of entitiess.\n\"");
			last_print_time = level.time;
		}

		memset(&max_entity_failure, 0, sizeof(gentity_t));
		return &max_entity_failure;
	}
