...
// End
	...
	case IT_WEAPON:
		...
		//NOTE(nobo): Allow them to pickup any 3rd-slot weapon in ffa
		if (ffa)
		{
				...
				item->giTag == WP_STEN)
			{
				...
				if (COM_BitCheck(ps->weapons, WP_STEN) &&
					item->giTag != WP_STEN)
				{
					return qfalse;
				}
...
#if 0
			if (item->giTag == WP_VENOM ||
				item->giTag == WP_MAUSER ||
				item->giTag == WP_PANZERFAUST ||
				item->giTag == WP_FLAMETHROWER)
			{
				if (COM_BitCheck(ps->weapons, WP_VENOM) &&
					item->giTag != WP_VENOM)
				{
					return qfalse;
				}
				if (COM_BitCheck(ps->weapons, WP_MAUSER) &&
					item->giTag != WP_MAUSER)
				{
					return qfalse;
				}
				if (COM_BitCheck(ps->weapons, WP_PANZERFAUST) &&
					item->giTag != WP_PANZERFAUST)
				{
					return qfalse;
				}
				if (COM_BitCheck(ps->weapons, WP_FLAMETHROWER) &&
					item->giTag != WP_FLAMETHROWER)
				{
					return qfalse;
				}
			...
			if (item->giTag == WP_LUGER ||
				item->giTag == WP_COLT)
			{
				if (COM_BitCheck(ps->weapons, WP_LUGER) &&
					item->giTag != WP_LUGER)
				{
					return qfalse;
				}
				if (COM_BitCheck(ps->weapons, WP_COLT) &&
					item->giTag != WP_COLT)
				{
					return qfalse;
				}
