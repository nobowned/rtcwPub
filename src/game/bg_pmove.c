...
static qboolean PM_CheckJump(void) {
	...
	if (pml.groundTrace.entityNum == ENTITYNUM_WORLD && (pml.groundTrace.surfaceFlags & SURF_SLICK)) {
		pm->ps->velocity[2] += 50;
	}
...
static void PM_FinishWeaponChange(void) {
	...
	int medic_has_ammo_pack = g_medicAmmoPack.integer;
#else
	int medic_has_ammo_pack = 0;
#endif
	...
	// g_medicAmmoPack.. we clear out ammo so when they return to weaponslot 6 medpack is always selected.
	// this is just more intuitive for the deathmatch medic crowd.
	if (medic_has_ammo_pack && pm->ps->stats[STAT_PLAYER_CLASS] == PC_MEDIC)
	{
		if (oldweapon == WP_AMMO && newweapon != WP_MEDKIT)
		{
			COM_BitClear(pm->ps->weapons, WP_AMMO);
			pm->ps->ammo[BG_FindAmmoForWeapon(WP_AMMO)] = 0;
			pm->ps->ammoclip[BG_FindClipForWeapon(WP_AMMO)] = 0;
		}
		else if (newweapon == WP_MEDKIT)
		{
			COM_BitSet(pm->ps->weapons, WP_AMMO);
			pm->ps->ammo[BG_FindAmmoForWeapon(WP_AMMO)] = 0;
			pm->ps->ammoclip[BG_FindClipForWeapon(WP_AMMO)] = 1;
		}
	}
...
==============
*/
qboolean PM_AutoReloads(int weapon) {
	switch (weapon) {
	case WP_MP40:
	case WP_THOMPSON:
	case WP_STEN:
...
void PM_CheckForReload(int weapon) {
	...
	// L0 
	// autoReload off
	if (pm->ps->noReload == qfalse && !(pm->ps->ammoclip[BG_FindClipForWeapon(weapon)]) &&	// clip is empty...
		pm->ps->ammo[BG_FindAmmoForWeapon(weapon)])
	{
		PM_BeginWeaponReload(weapon);
	}
	// autoReload on
	else if (!(PM_AutoReloads(pm->ps->weapon)) && !(pm->ps->ammoclip[BG_FindClipForWeapon(weapon)]) &&	// clip is empty...
...
static void PM_Weapon(void) {
	...
	if (pm->gametype >= GT_WOLF) {
		...
		if (pm->ps->weapon == WP_AMMO) {
			if (pm->ps->stats[STAT_PLAYER_CLASS] == PC_MEDIC) {
				if (pm->cmd.serverTime - pm->ps->classWeaponTime < (pm->medicChargeTime*0.75))
					return;
			}
			else {
...
// end
	...
	if (pm->ps->weapon) {
		...
		if (ammoNeeded > ammoAvailable)
		{
			...
			// L0 - noReload
			if ((pm->ps->noReload == qtrue) && PM_AutoReloads(pm->ps->weapon) && !(pm->cmd.wbuttons & WBUTTON_RELOAD)) {
				reloading = qfalse;
			}
...
void PM_UpdateViewAngles(playerState_t *ps, usercmd_t *cmd, void (trace)(trace_t *results, const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int passEntityNum, int contentMask)) {	//----(SA)	modified
	...
	if (ps->eFlags & EF_MG42_ACTIVE) {
		...
		arcMax = ps->varc;
		arcMin = ps->varc / 2;
		arcDiff = AngleNormalize180(ps->viewangles[PITCH] - ps->centerangles[PITCH]);


		if (arcDiff > arcMin) {
			ps->viewangles[PITCH] = AngleNormalize180(ps->centerangles[PITCH] + arcMin);

			// Set delta_angles properly
			ps->delta_angles[PITCH] = ANGLE2SHORT(ps->viewangles[PITCH]) - cmd->angles[PITCH];
		}
		else if (arcDiff < -arcMax) {
			ps->viewangles[PITCH] = AngleNormalize180(ps->centerangles[PITCH] - arcMax);
		...
		// yaw (harc)
		arcMin = arcMax = ps->harc;
		arcDiff = AngleNormalize180(ps->viewangles[YAW] - ps->centerangles[YAW]);


		if (arcDiff > arcMin) {
			ps->viewangles[YAW] = AngleNormalize180(ps->centerangles[YAW] + arcMin);

			// Set delta_angles properly
			ps->delta_angles[YAW] = ANGLE2SHORT(ps->viewangles[YAW]) - cmd->angles[YAW];
		}
		else if (arcDiff < -arcMax) {
			ps->viewangles[YAW] = AngleNormalize180(ps->centerangles[YAW] - arcMax);
	...
	else
	{
		...
		if (pm->ps->powerups[PW_NOFATIGUE])	// (SA) recharge at 2x with stamina powerup
			...
			if (pm->gametype != GT_SINGLE_PLAYER) {
				...
				if (g_crouchStaminaBoost.integer > 0 && (pm->ps->pm_flags & PMF_DUCKED))
					pm->ps->sprintTime += g_crouchStaminaBoost.integer * pml.frametime;
