...
void Q_RemoveTrailingCharacter(char *string, char c) {
...
void Q_RemoveLeadingCharacter(char *string, char c) {
	char *s = string;

	while (*s == c)
		s++;

	int newlen = strlen(string) - (s - string);
	memmove(string, s, newlen);
	string[newlen] = 0;
}

char *Q_ChrReplace(char *haystack, char needle, char replacement) {
	char *replacer = haystack;

	while ((replacer = strchr(replacer, needle)) != NULL)
		*replacer++ = replacement;

	return haystack;
}

char *Q_StrReplace(char *haystack, char *needle, char *replacement) {
	...
	if (*replacement) {
		Q_strncpyz(newStr, replacement, sizeof(newStr));
	}
...
qboolean Q_FindToken(char *haystack, char *needle) {
	if (haystack != NULL && strlen(haystack) &&
		needle != NULL && strlen(needle)) {
		char *token;
...
/*
===========
Q_SanitizeClientTextInput
============
*/
int Q_SanitizeClientTextInput(const char *in, char *out, int outSize, qboolean limit_spaces) {
	int outpos = 0, colorlessLen = 0, spaces = 0, i;
	qboolean invalid_character_found;
	char in_copy[MAX_STRING_CHARS];

#define INVALID_CHAR_LENGTH 10

	Q_strncpyz(in_copy, in, sizeof(in_copy));
	char *in2 = in_copy;

	static int invalid_characters[INVALID_CHAR_LENGTH] = { 4, 5, 10, 12, 13, 14, 15, 22, ' ' };

	// discard leading invalid characters
	invalid_character_found = qtrue;
	while (invalid_character_found && in2 && *in2) {
		invalid_character_found = qfalse;
		for (i = 0; i < INVALID_CHAR_LENGTH; ++i) {
			if (*in2 == invalid_characters[i]) {
				invalid_character_found = qtrue;
				in2++;
				break;
			}
	...
	for (; *in2 && outpos < outSize - 1; in2++) {
		...
		if (*in2 == ' ' && limit_spaces) {
			...
			if (Q_IsColorString(&out[outpos - 1])) {
				if (!Q_IsColorString2(&out[outpos - 1])) {
					out[outpos] = ColorIndex(out[outpos]) + '0';
				}
	...
	// discard trailing invalid characters
	for (i = 0; i < INVALID_CHAR_LENGTH; ++i) {
		Q_RemoveTrailingCharacter(out, invalid_characters[i]);
	}
	Q_RemoveTrailingCharacter(out, '^');
