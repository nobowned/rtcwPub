...
const char *TeamNameColored(int team) {
	if (team == TEAM_RED)
		return S_COLOR_RED "Axis";
	else if (team == TEAM_BLUE)
		return S_COLOR_BLUE "Allies";
	else if (team == TEAM_SPECTATOR)
		return S_COLOR_YELLOW "Spectators";
	return S_COLOR_WHITE "Free";
}
...
void TeamplayInfoMessage( gentity_t *ent ) {
	...
	for (i = 0, cnt = 0; i < level.numNonDisconnectedClients && cnt < TEAM_MAXOVERLAY; i++) {
...
void checkpoint_spawntouch(gentity_t *self, gentity_t *other, trace_t *trace) {
	...
	if (g_gametype.integer != GT_WOLF_CP) {
		if (g_flagRetake.integer == 0) {
			CPx(other->s.clientNum, "cp \"Flag Capturing is disabled on this server^1!\n\"1");
			return;
		}
		else {
			// First we check if flagRetake is disabled..if it isn't
			// balance check comes first.
			int flagbalance = FlagBalance();
			...
			if (flagbalance == 1 && other->client->sess.sessionTeam != TEAM_RED)
			{
				...
				{
					CPx(other->s.clientNum, va("cp \"Uneven teams! Your team currently cannot claim the flag.\n\"2"));
					return;
				}
			...
			if (self->s.frame == WCP_ANIM_NOFLAG) {
				...
				if (level.flagTaken >= g_flagRetake.integer && g_flagRetake.integer != -1){
					CPx(other->s.clientNum, va("cp \"Flag Retake limit ^1%d ^7has been reached^1!\n\"2", g_flagRetake.integer));
					return;
				}
				// Flags can be used to intentionally give away position on custom (large) maps but no scoring for it..
				if (!g_deathmatch.integer)
...
void handleTeamLocks( int team ) {
	...
	if (teamInfo[team].team_lock == qtrue)
	{
		if ((team == TEAM_RED && axisPlayers.integer < 1) || 
			(team == TEAM_BLUE && alliedPlayers.integer < 1))	{
