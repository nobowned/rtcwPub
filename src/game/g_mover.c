...
#define JITTER_MAX  ( check->r.maxs[0] / 2.0 )
	...
	pushed_p->ent = check;
	pushed_p->groundEntityNum = check->s.groundEntityNum;
	...
	check->s.groundEntityNum = (pushed_p - 1)->groundEntityNum;
	block = G_TestEntityPosition(check);
	if (!block) {
		check->s.groundEntityNum = -1;
		pushed_p--;
	...
	for (e = 0; e < moveEntities; e++) {
		...
		*obstacle = check;
		...
		for (p = pushed_p - 1; p >= pushed; p--) {
			...
			p->ent->s.groundEntityNum = p->groundEntityNum;
			if (p->ent->client) {
...
void Blocked_Door( gentity_t *ent, gentity_t *other ) {
	...
	if(other) {
		if (!G_IsEntityAutomg42(other)) {
			...
			G_Damage(other, ent, ent, NULL, NULL, ent->damage, 9999, MOD_CRUSH);
		}
	}
...
void SP_func_leaky( gentity_t *ent ) {
	// NOTE(nobo): None of this is needed.
	/*
	if ( ent->model2 ) {
		ent->s.modelindex2 = G_ModelIndex( ent->model2 );
	}
...
void func_explosive_explode(gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod)
{
	...
	self->r.contents = self->clipmask = 0;
...
/*
==============
bot_func_explosive_touch
==============
*/
void bot_func_explosive_touch(gentity_t *self, gentity_t *other, trace_t *trace)
{
	if (self->takedamage && other && other->r.svFlags & SVF_BOT)
		func_explosive_explode(self, self, other, self->damage, 0);
}
...
void SP_func_explosive (gentity_t *ent)
{
	...
	if (trap_Cvar_VariableIntegerValue("bot_enable")) {
		if (!ent->touch)
			ent->touch = bot_func_explosive_touch;
		ent->nextthink = 0;
	}
