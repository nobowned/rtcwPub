...
// Nobo - Defined, but not currently used.
#if 0
/*
===============
...
void G_AddRandomBot( int team ) {
	int		i, n, skill, bots_count;
	char	netname[36], *value, *type;
	int		bots[MAX_BOTS];
	gclient_t	*cl;
	...
	for ( n = 0; n < g_numBots ; n++ ) {
		...
		for ( i = trap_Cvar_VariableIntegerValue("sv_privateClients"); i < g_maxclients.integer ; i++ ) {
			cl = level.clients + i;
			SanitizeString(cl->pers.netname, netname, qfalse);
	...
	n = bots[rand() % bots_count];
	value = Info_ValueForKey(g_botInfos[n], "name");
	type = Info_ValueForKey(g_botInfos[n], "type");
	skill = trap_Cvar_VariableIntegerValue("bot_skill");

	strncpy(netname, value, sizeof(netname) - 1);
	netname[sizeof(netname) - 1] = '\0';
	Q_CleanStr(netname);

	trap_SendConsoleCommand(EXEC_INSERT, va("addbot %s %i %i %i %i\n", netname, skill, team, strlen(type) ? atoi(type) : 1, 0));
}
...
int G_RemoveRandomBot( int team ) {
	...
	for ( i=0 ; i< g_maxclients.integer ; i++ ) {
		...
		Q_CleanStr(netname);
		trap_DropClient(cl - level.clients, "disconnected");
		return qtrue;
	}
	return qfalse;
}
...
void G_CheckMinimumPlayers( void ) {
	int minplayers;
	int humanplayers, botplayers, totalplayers;
	qboolean should_add;
	...
	if (g_gametype.integer >= GT_TEAM) {
		should_add = !(g_noTeamSwitching.integer && g_gamestate.integer == GS_PLAYING);
		...
		//
		if (totalplayers < minplayers && should_add) {
			G_AddRandomBot( TEAM_RED );
		} else if (totalplayers > minplayers && botplayers) {
...
===============
*/
static void G_AddBot(const char *name, int skill, int team, int type, int delay) {
	int				clientNum;
	char			*botinfo;
	gentity_t		*bot;
	char			*key;
	...
	char			modified_name[MAX_NETNAME];
	...
	Info_SetValueForKey( userinfo, "team", va("%i", team) );
	Info_SetValueForKey( userinfo, "class", va("%i", type) );
	Info_SetValueForKey( userinfo, "cg_autoactivate", va("%i", 1) );
	...
	s = Info_ValueForKey( botinfo, "funname" );
	if ( !*s ) {
		s = Info_ValueForKey( botinfo, "name" );
	}
	Q_strncpyz(modified_name, va("[^3BOT^7] %s",  s), sizeof(modified_name));
	Info_SetValueForKey(userinfo, "name", modified_name);
	...
	connectResponse = ClientConnect(clientNum, qtrue, qtrue);
	if (connectResponse) {
		G_LogPrintf("[Bot] ClientConnect Refused: %s\n", connectResponse);
		return;
	}
...
void Svcmd_AddBot_f( void ) {
	...
	if ( !trap_Cvar_VariableIntegerValue( "bot_enable" ) ) {
		G_Printf( "Bots are disabled!\n" );
		return;
	}

	// name
	trap_Argv( 1, name, sizeof( name ) );
	if ( !name[0] || trap_Argc() < 5 ) {
		G_Printf("Usage: Addbot [botname] [skill 1-4] [team] [type 0-3] [msec delay]\n");
		return;
	}
	...
	if (!string[0]) {
		G_Printf("Svcmd_AddBot_f: Bot team is required.\n");
		return;
	}
	team = atoi(string);
	...
	G_AddBot( name, skill, team, type, delay );
...
char *G_GetBotInfoByName( const char *name ) {
	int		n;
	char	*value;
	char	clean1[MAX_NETNAME];
	char	clean2[MAX_NETNAME];

	SanitizeString(name, clean1, qfalse);

	for ( n = 0; n < g_numBots ; n++ ) {
		value = Info_ValueForKey( g_botInfos[n], "name" );
		SanitizeString(value, clean2, qfalse);

		if (!Q_stricmp(clean1, clean2)) {
			return g_botInfos[n];
		}
	}
...
void G_InitBots( qboolean restart ) {
	...
	trap_Cvar_Register(&bot_skill, "bot_skill", "0", CVAR_SERVERINFO);


	// NOTE(nobo): Quake3 single player stuff that'll never be used in rtcw single player.
	// Only keeping around since arenas may be useful in multiplayer at some point.
#if 0	
	int			fragLimit;
	int			timeLimit;
