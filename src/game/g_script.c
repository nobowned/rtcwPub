...
void G_Script_ScriptLoad( void )
{
	...
	char			scriptlist[MAX_STRING_CHARS];
	char			*scriptname;
	vmCvar_t		mapname;
	fileHandle_t	f;
	int				len, numscripts, scriptlen, i;
	...
	if (g_deathmatch.integer) {
		numscripts = trap_FS_GetFileList("dmscripts", ".script", scriptlist, sizeof(scriptlist));
		scriptname = scriptlist;
		for (i = 0; i < numscripts; i++, scriptname += scriptlen + 1) {
			COM_StripExtension(scriptname, noext);
			if (!Q_stricmp(noext, mapname.string)) {
				Q_strncpyz(filename, va("dmscripts/%s", scriptname), sizeof(filename));
				break;
			}
			scriptlen = strlen(scriptname);
		}
	}
...
void script_mover_blocked( gentity_t *ent, gentity_t *other ) {
	...
	if ( !other->client && !G_IsEntityAutomg42(other) ) {
