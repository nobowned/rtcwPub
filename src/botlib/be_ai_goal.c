...
// map_name.rts in maps folder.
void BotInitRoutes( const char *mapname ) {
	bot_route_t route;
	script_t *script;
	token_t token;
	char rtspath[MAX_PATH];
	int i, num_routes_start;

	num_routes_start = 0;
	memset(routes, 0, sizeof(routes));

	num_routes_start = num_routes;

	if (!AAS_Loaded()) {
		return;
	}

	Com_sprintf(rtspath, MAX_PATH, "maps/%s.rts", mapname);
	PS_SetBaseFolder("");
	script = LoadScriptFile(rtspath);
	if (!script) {
		botimport.Print(PRT_ERROR, "BotInitRoutes: couldn't load %s", rtspath);
		return;
	}

	while (PS_CheckTokenString(script, "{")) {
		memset(&route, 0, sizeof(bot_route_t));

		while (PS_ReadToken(script, &token) && strcmp(token.string, "}")) {
			if (!strcmp(token.string, "path")) {
				...
				do {
					for (i = 0; i < 3; ++i) {
						route.path[route.path_length][i] = ReadSignedInt(script);
					}
					route.path_length++;
				} while (PS_ReadToken(script, &token) && !strcmp(token.string, ",") && strcmp(token.string, "}"));
			}
			else if (!strcmp(token.string, "team")) {
				if (!PS_ReadToken(script, &token)) {
					return;
				}

				route.team = token.intvalue;
			}
			else if (!strcmp(token.string, "type")) {
				if (!PS_ReadToken(script, &token)) {
					return;
				}

				route.type = token.intvalue;
			}
			else {
				return;
			}
		...
		route.index = num_routes;
		routes[num_routes++] = route;
	}

	botimport.Print(PRT_MESSAGE, "found %d bot routes\n", num_routes - num_routes_start);
}
//===========================================================================
//
// Parameter:				-
...
void BotPushRouteGoal(bot_route_t *route, int pathIndex, int extraFlags, int goalstate) {
	bot_goal_t goal;
	float random;
	...
	random = random();
	if (route->type != ROUTE_TYPE_STARTING && random < 0.1) {
		extraFlags |= GFL_WALK;
	}
	else if (random > 0.1 && random < 0.7) {
		extraFlags |= GFL_SPRINT;
	}

	memset(&goal, 0, sizeof(bot_goal_t));
	goal.iteminfo = route->index;
	goal.flags = (GFL_ROUTE | GFL_NOSLOWAPPROACH | extraFlags);
	goal.number = pathIndex;
	VectorCopy(mins, goal.mins);
	VectorCopy(maxs, goal.maxs);
	VectorCopy(route->path[pathIndex], goal.origin);
	goal.areanum = AAS_BestReachableArea(goal.origin, mins, maxs, goal.origin);
	BotPushGoal(goalstate, &goal);
}
int BotChooseRouteGoal(int goalstate, int team, vec3_t origin, bot_goal_t *current_goal, int seconds_doing_route) {
	int i, j, ent;
	int best_distance, best_path_index;
	bot_goalstate_t *bot_goal_state;
	bot_route_t *current_route, *best_route;
	vec3_t enemy_base_direction, node_direction, enemy_base_origin;
	char classname[MAX_EPAIRKEY];
	qboolean enemy_base_found;

	best_route = NULL;
	current_route = NULL;
	best_distance = INT_MAX;
	best_path_index = 0;
	enemy_base_found = qfalse;

	bot_goal_state = BotGoalStateFromHandle(goalstate);
	if (bot_goal_state == NULL) {
		return qfalse;
	}

	// find direction towards enemy base.
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)) {
		if (!AAS_ValueForBSPEpairKey(ent, "classname", classname, MAX_EPAIRKEY)) {
			continue;
		}
		if (!AAS_VectorForBSPEpairKey(ent, "origin", enemy_base_origin)) {
			continue;
		}
		if ((team == ROUTE_TEAM_RED && strcmp(classname, "team_CTF_blueplayer") == 0) ||
			(team == ROUTE_TEAM_BLUE && strcmp(classname, "team_CTF_redplayer") == 0)) {
			VectorSubtract(enemy_base_origin, origin, enemy_base_direction);
			VectorNormalizeFast(enemy_base_direction);
			enemy_base_found = qtrue;
			break;
		}
	}

	if (current_goal->flags & GFL_ROUTE) {
		current_route = &routes[current_goal->iteminfo];
		if (current_goal->flags & GFL_ROUTE_REVERSE) {
			if (current_goal->number != 0) {
				BotPushRouteGoal(current_route, current_goal->number - 1, current_goal->flags, goalstate);
				return qtrue;
			}
		}
		else {
			if (current_goal->number != current_route->path_length) {
				BotPushRouteGoal(current_route, current_goal->number + 1, current_goal->flags, goalstate);
				return qtrue;
			}
		}
	...
	// TODO: Sample multiple nodes of multiple routes/paths.

	if (current_route == NULL) {
		for (i = 0; i < num_routes; ++i) {
			bot_route_t *route = routes + i;

			if (route->team != team) {
				continue;
			}
			
			for (j = 0; j < route->path_length; ++j) {
				VectorSubtract(route->path[j], origin, node_direction);
				int distance = VectorNormalize(node_direction);
				if ((!enemy_base_found || fabs(DotProduct(enemy_base_direction, node_direction)) > 0.7) && distance < best_distance) {
		...
		BotPushRouteGoal(best_route, best_path_index, 0, goalstate);
		return qtrue;
	}
	else { // we're ending our current route.
		int pathIndex = 0;
		int extraFlags = 0;

		switch (current_route->type) {
		case ROUTE_TYPE_STARTING:
			// find primary route.
			for (i = 0; i < num_routes; ++i) {
				bot_route_t *route = routes + i;
				...
				if (route->type != ROUTE_TYPE_PRIMARY) {
					continue;
				}

				int distance = VectorDistance(origin, route->path[0]);
		...
		case ROUTE_TYPE_PRIMARY:
			...
			for (i = 0; i < num_routes; ++i) {
				...
				if (route->type != ROUTE_TYPE_ENDING) {
					continue;
				}
				...
				if (distance < best_distance) {
					best_route = route;
					best_distance = distance;
					extraFlags = GFL_ROUTE_CAMP;
				}
			}
			break;
		case ROUTE_TYPE_ENDING:
			if (seconds_doing_route > 30) {
				// find a starting route and reverse through it
				for (i = 0; i < num_routes; ++i) {
					bot_route_t *route = routes + i;
					...
					if (route->type != ROUTE_TYPE_STARTING) {
						continue;
					}

					best_route = route;
					extraFlags = GFL_ROUTE_REVERSE;
					pathIndex = route->path_length - 1;
					break;
				}
			}
			else {
				// camp current ending route.
				extraFlags = GFL_ROUTE_CAMP;
			}
			break;
		}
		...
		if (best_route == NULL) {
			// couldn't find a route.. so reverse the current one.
			if (!(current_goal->flags & GFL_ROUTE_REVERSE)) {
				extraFlags |= GFL_ROUTE_REVERSE;
				pathIndex = current_route->path_length - 1;
			}
			BotPushRouteGoal(current_route, pathIndex, extraFlags, goalstate);
			return qtrue;
		}
		else {
			BotPushRouteGoal(best_route, pathIndex, extraFlags, goalstate);
			return qtrue;
		}
	}
...
int GetRouteAtIndex(int index, bot_route_t *route) {
	if (index < 0 || index >= num_routes) {
		return -1;
	}

	memcpy(route, &routes[index], sizeof(bot_route_t));
