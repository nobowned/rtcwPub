...
void BotMoveToGoal( bot_moveresult_t *result, int movestate, bot_goal_t *goal, int travelflags ) {
	...
	// if the goal wants walking
	if (goal->flags & GFL_WALK) {
		ms->moveflags |= MFL_WALK;
	}
	// if the goal wants sprinting
	else if (goal->flags & GFL_SPRINT) {
		ms->moveflags |= MFL_SPRINT;
	}
	  //if swimming
	if ( AAS_Swimming( ms->origin ) ) {
		ms->moveflags |= MFL_SWIMMING;
		// don't walk if swimming
		ms->moveflags &= ~MFL_WALK;
	}
	//if against a ladder
	if ( AAS_AgainstLadder( ms->origin, ms->areanum ) ) {
		ms->moveflags |= MFL_AGAINSTLADDER;
		// don't walk if laddering
		ms->moveflags &= ~MFL_WALK;
	}
...
// done.
		...
		if ( !reachnum ) {
			...
			// TODO(nobo): If the bot is stuck in a dead zone (areanum == 0), then set areanum back to lastareanum
			// A counter or timer may be needed, to prevent bots from ping ponging between two areas
			if (ms->areanum == 0)
				ms->areanum = ms->lastgoalareanum;
			if (ms->areanum == 0)
				ms->areanum = ms->lastareanum;
			// NOTE(nobo): The above four lines of code are not the final fix! They seem to work, but are not properly tested!
