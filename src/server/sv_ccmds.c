...
================
*/
static qboolean SV_TransitionGameState( gamestate_t new_gs, gamestate_t old_gs ) {
...
static void SV_MapRestart_f( void ) {
	...
	qboolean		isBot;
	int				delay;
	gamestate_t		new_gs, cur_gs;     // NERVE - SMF
	...
	cur_gs = atoi(Cvar_VariableString("gamestate"));

	if (delay) {
		// NOTE(nobo): Clear it first, otherwise can't map_restart with the same value (cvar restriction..)
		Cvar_Set("g_warmup", "-1");
		Cvar_Set("g_warmup", va("%i", delay));
		// NOTE(nobo): If we're already counting down just modify the warmup time instead of fully restarting the match
		if (cur_gs == GS_WARMUP_COUNTDOWN)
			return;
		Cvar_Set("gamestate", va("%i", GS_WARMUP));
	} else {
		if (Cmd_Argc() > 2) {
		...
		if (!SV_TransitionGameState(new_gs, cur_gs)) {
			return;
		}
	}
...
void SV_AddOperatorCommands( void ) {
	...
	Cmd_AddCommand("resendStreamedPacket", Sys_ResendStreamedPacket);
}
