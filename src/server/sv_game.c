...
Called from game module to get a clients ip address. This prevents any and all userinfo ip spoofing.
===============
*/
void SV_GetClientIp(int clientNum, char *ip_out, int ip_out_length) {
	client_t *cl;
...
/*
===============
SV_GetClientProtocol

Called from game module to get a clients protocol/version (1.0[57]/1.4[60]).
===============
*/
int SV_GetClientProtocol(int clientNum) {
	client_t *cl;
...
Called from game module to set a clients name in the server thingy.
===============
*/
void SV_SetClientName(int clientNum, const char *name) {
	client_t *cl;
...
Called from game module to log a message to a file.
===============
*/
void SV_LogToFile(char* q_path, char* message) {
	...
	FS_FOpenFileByMode(q_path, &f, FS_APPEND);

	varLine = va("%s\r\n", message);

	FS_Write(varLine, strlen(varLine), f);
	FS_FCloseFile(f);
}
...
int SV_GameSystemCalls( int *args ) {
	...
	case G_SEND_STATS:
		SV_SendGlobalStats(VMA(1));
		return 0;
	case G_GLOBAL_STATS_CONNECT:
		SV_GlobalStatsConnect();
		return 0;
	case G_GLOBAL_STATS_DISCONNECT:
	...
	case G_LOG_TO_FILE:
		...
		return 0;
	case G_CLOSE_COMMON_LOG_FILE:
		Com_CloseLogFile();
		return 0;
	case G_COMMON_LOG_FILE_EXISTS:
		return Com_LogFileExists();
	...
	case BOTLIB_EA_SAY_TEAM:
		...
		return 0;
	case BOTLIB_EA_VOICE:
		botlib_export->ea.EA_Voice(args[1], VMA(2));
		return 0;
	case BOTLIB_EA_VOICE_TEAM:
		botlib_export->ea.EA_VoiceTeam(args[1], VMA(2));
		return 0;
	...
	case BOTLIB_EA_GESTURE:
		...
		return 0;
	case BOTLIB_EA_ACTIVATE:
		botlib_export->ea.EA_Activate(args[1]);
		return 0;
	...
	case BOTLIB_AI_CHOOSE_ROUTE_GOAL:
		return botlib_export->ai.BotChooseRouteGoal(args[1], args[2], VMA(3), VMA(4), args[5]);
	case BOTLIB_AI_GET_ROUTE_AT_INDEX:
		return botlib_export->ai.GetRouteAtIndex(args[1], VMA(2));
...
qboolean SV_GetTag( sharedEntity_t *ent, clientAnimationInfo_t *animInfo, char* tagname, orientation_t *or ) {
	...
	return CL_GetTag( ent->s.number, tagname, or );
#else
	vec3_t tempAxis[3];
	vec3_t org;
	int i;

	if (SV_LerpTag(or, animInfo, tagname) < 0)
		return qfalse;

	VectorCopy(ent->r.currentOrigin, org);

	for (i = 0; i < 3; i++) {
		VectorMA(org, or->origin[i], animInfo->legsAxis[i], org);
	}

	VectorCopy(org, or->origin);

	// rotate with entity
	MatrixMultiply(animInfo->legsAxis, or ->axis, tempAxis);
