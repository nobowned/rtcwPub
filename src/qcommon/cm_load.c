...
/*
=================
CMod_LoadEntityString - Nobo's old way
=================
void CMod_LoadEntityString(const char *path, lump_t *l) {
	int *buf = NULL;
	char *entities = "";
	int length = 0;
	char mapname[MAX_QPATH];

	char *extension = strstr(path, "bsp");
	if (extension)
	{
		Q_strncpyz(mapname, path, extension - path);
		Q_strcat(mapname, sizeof(mapname), ".spawns");
		Q_strlwr(mapname);

		length = FS_ReadFile(mapname, (void **)&buf);
		if (buf) entities = (char *)buf;
	}

	cm.entityString = Hunk_Alloc( l->filelen + length, h_high );
	cm.numEntityChars = l->filelen + length;
	memcpy( cm.entityString, cmod_base + l->fileofs, l->filelen );
	strcat( cm.entityString, entities );
...
/*
=================
sswolf - replace mod method with jk's.
Allows full overwrite and is much more compact.
Note: takes ffa into account.
...
qboolean CMod_LoadCustomEntityString(const char* name) {
	...
	if (Cvar_VariableIntegerValue("g_ffa"))
	{
		filename = va("%s_ffa.spawns", noext);
	}
	else
	{
	...
	if (file && entFileLen > 0)
	{
		...
		Com_Printf(va("rtcwPub: Loaded entities from %s\n", filename));
		return qtrue;
	}
...
void CM_LoadMap( const char *name, qboolean clientload, int *checksum ) {
	...
	*checksum = last_checksum;
	...
	CMod_LoadEntityString( &header.lumps[LUMP_ENTITIES], name ); // - Nobo's old way // sswolf - back to it
