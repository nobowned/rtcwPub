...
static float CG_DrawRespawnTimer(float y) {
	...
	Vector4Set(color, 0.63f, 0.47f, 0.25f, cg_hudAlpha.value);
	trap_R_SetColor(color);
...
static void CG_DrawCrosshair(void) {
	...
	static int  xHairColorModifiedCount = 0;
	static vec4_t xHairColor = { 1, 1, 1, 1 };
	...
	if ( cg_crosshairHealth.integer ) {
		...
		if (xHairColorModifiedCount != cg_crosshairColor.modificationCount) {
			xHairColorModifiedCount = cg_crosshairColor.modificationCount;
			memcpy(xHairColor, g_color_table[((unsigned)cg_crosshairColor.integer) % ArrayLength(g_color_table)], sizeof(xHairColor));
		}
